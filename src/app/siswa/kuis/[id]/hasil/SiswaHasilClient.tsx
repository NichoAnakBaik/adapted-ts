    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="space-y-6">
        ...
        {(() => {
          ...
          if (exam.is_final) {
            const pillars = ['READING', 'LISTENING', 'SPEAKING', 'WRITING', 'MULTIPLE_CHOICE'];
            return (
              <div className="space-y-8">
                {pillars.map((pillar) => {
                  const pillarQs = exam.questions.filter((q: any) => q.type === pillar);
                  if (pillarQs.length === 0) return null;
                  return (
                    <div key={pillar} className="space-y-4">
                      <h3 className="text-lg font-black text-namsan-primary border-b-2 border-namsan-primary/20 pb-2 mb-4 uppercase tracking-widest flex items-center gap-2">
                        Pilar: {pillar.replace('_', ' ')}
                      </h3>
                      <div className="space-y-6">
                        {pillarQs.map((q: any, i: number) => renderQuestion(q, i))}
                      </div>
                    </div>
                  );
                })}
          ...
          return exam.questions.map((q: any, index: number) => renderQuestion(q, index));
        })()}
      </div>
    </div>
  );
