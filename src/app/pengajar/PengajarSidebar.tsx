...
import { LayoutDashboard, BookOpen, PenTool, MessageSquare, Award, LineChart, Users, LogOut } from "lucide-react";
...
export function PengajarSidebar({ user }: { user: any }) {
  ...
  const menuItems = [
    ...
    { name: "Manajemen Modul", href: "/pengajar/modul", icon: BookOpen },
    { name: "Kuis & Evaluasi", href: "/pengajar/kuis", icon: PenTool },
    ...
    <aside className="w-64 bg-white border-r border-gray-200 flex flex-col h-screen shrink-0">
      ...
      <div className="mt-auto border-t border-gray-100 p-4 shrink-0">
        ...
        <div className="flex items-center gap-3 mb-4 px-2">
          <div className="w-10 h-10 rounded-full bg-namsan-primary flex items-center justify-center text-white font-bold text-sm">
            {user?.username?.substring(0, 2).toUpperCase() || "PE"}
          </div>
          <div>
            <p className="text-sm font-bold text-namsan-text">{user?.username || "Pengajar"}</p>
            <p className="text-xs text-namsan-text-muted capitalize">{user?.role?.toLowerCase() || "pengajar"}</p>
          </div>
        </div>
        {/* Logout Button */}
