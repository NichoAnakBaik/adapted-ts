...
import { ClipboardList, Users, ArrowRight, LayoutDashboard } from "lucide-react";
import Link from "next/link";

export default function PengajarKuisIndexClient({ classes }: { classes: any[] }) {
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-namsan-text-muted">Pilih kelas di bawah ini untuk melihat dan membuat kuis.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredClasses.map((c) => (
          <div key={c.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col hover:border-namsan-primary transition-colors group">
            ...
            <Link href={`/pengajar/kuis/kelas/${c.id}`} className="mt-auto w-full bg-namsan-soft hover:bg-namsan-primary text-namsan-primary hover:text-white font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors">
