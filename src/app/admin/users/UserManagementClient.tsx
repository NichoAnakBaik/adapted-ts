        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white p-4 md:p-6 rounded-2xl shadow-xl w-full max-w-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4 border-b pb-3">
              <h2 className="text-lg md:text-xl font-bold text-gray-800">Buat Pengguna Baru</h2>
            ...
            <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">Nama Lengkap</label>
                <input type="text" name="nama_lengkap" required className="w-full p-2 md:p-2.5 border rounded-lg text-sm md:text-base" />
              </div>
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">Username</label>
                <input type="text" name="username" required className="w-full p-2 md:p-2.5 border rounded-lg text-sm md:text-base" />
              </div>
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">Password</label>
                <input type="password" name="password" required className="w-full p-2 md:p-2.5 border rounded-lg text-sm md:text-base" />
              </div>
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">Role</label>
                <select name="role" required className="w-full p-2 md:p-2.5 border rounded-lg bg-white text-sm md:text-base">
              ...
              <div className="md:col-span-2 mt-2 md:mt-4 flex flex-col-reverse sm:flex-row justify-end gap-2 md:gap-3 pt-3 md:pt-4 border-t border-gray-100">
                <button type="button" onClick={() => setShowForm(false)} className="w-full sm:w-auto px-4 md:px-5 py-2 md:py-2.5 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg font-bold transition-colors text-sm md:text-base">
                  Batal
                </button>
                <button type="submit" className="w-full sm:w-auto bg-namsan-text hover:bg-namsan-text/90 text-white font-bold py-2 md:py-2.5 px-5 md:px-6 rounded-lg transition-colors text-sm md:text-base">
                  Simpan Pengguna
                </button>
              </div>
            </form>
                ...
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors text-xs md:text-base">
                    ...
                    <td className="p-3 md:p-4 text-right whitespace-nowrap">
                      <button onClick={() => handleDelete(u.id)} className="p-1 md:p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
