...
export default function AdminUjianDetailClient({ exam, initialEligibleStudents }: { exam: any, initialEligibleStudents: any[] }) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      <Link href={`/admin/ujian/kelas/${exam.class_id}`} className="inline-flex items-center gap-2 text-gray-500 hover:text-namsan-primary font-medium transition-colors">
        <ArrowLeft className="w-4 h-4" /> Kembali ke Daftar Ujian Kelas
      </Link>
                    ...
                    <div className="md:col-span-2">
                      ...
                      {imageUploadType === "NEW" ? (
                        ...
                        <div className="space-y-3">
                          <p className="text-xs text-gray-500 mb-2">Pilih gambar yang sudah pernah diupload di soal lain pada ujian ini.</p>
                          ...
                          {existingImages.length === 0 && (
                            <p className="text-sm text-red-500">Belum ada gambar yang terupload di ujian ini.</p>
                          )}
                        </div>
                      )}
                    </div>
