  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center">
        ...
        <p className="text-gray-500 mb-8 max-w-md mx-auto">Pilih kelas di bawah ini untuk melihat daftar ujian akhir yang tersedia pada kelas tersebut.</p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
          {classes.map((cls) => (
            <Link key={cls.id} href={`/admin/ujian/kelas/${cls.id}`} className="block p-6 rounded-2xl border border-gray-200 hover:border-namsan-primary hover:shadow-md transition-all group">
              ...
              <div className="mt-4 flex items-center gap-1 text-sm font-bold text-namsan-primary group-hover:gap-2 transition-all">
                Kelola Ujian <ArrowRight className="w-4 h-4" />
              </div>
            </Link>
          ))}
          {classes.length === 0 && (
